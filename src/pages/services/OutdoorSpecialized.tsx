      ...
      <section ref={containerRef} className="relative min-h-screen md:min-h-[90svh] flex flex-col justify-center overflow-hidden bg-background">
        ...
        <div className="container-sahli relative z-10 pt-20 md:pt-28 pb-10 md:pb-10 grid grid-cols-1 lg:grid-cols-2 gap-12 lg:gap-20 items-center">
          ...
          >
            ...
            <p className="text-lg md:text-xl lg:text-2xl !text-foreground/70 max-w-2xl mb-8 md:mb-12 w-full">
          ...
          >
            ...
            <div className="absolute bottom-6 md:bottom-8 left-6 md:left-8 right-6 md:right-8 p-5 md:p-6 rounded-2xl bg-background/80 backdrop-blur-md border border-white/10 shadow-lg">
              <div className="flex items-center gap-4">
                ...
                <div>
                  <div className="text-xs md:text-label !text-primary font-bold">{t('services.care.verifiedProvider')}</div>
                  <div className="text-xs md:text-label !text-foreground">{t('services.care.compassionateCare')}</div>
                </div>
              </div>
            </div>
          </motion.div>
            ...
            <Marquee speed={0.5} className="-mx-4 px-4">
              {[
                ...
                <div key={i} className="flex gap-4 items-start group shrink-0 w-[260px] md:w-auto p-4 md:p-0 rounded-2xl bg-background md:bg-transparent border border-border md:border-0 shadow-sm md:shadow-none">
                  ...
                  <span className="text-sm md:text-label !text-foreground/70 leading-snug group-hover:text-foreground transition-colors duration-500 font-medium">{rule}</span>
                </div>
              ))}
            </Marquee>
          </motion.div>
      ...
      <section className="section-spacing bg-background relative overflow-hidden">
        <div className="container-sahli">
          <div className="text-center max-w-3xl mx-auto mb-12 md:mb-16">
            ...
            <p className="text-base md:text-xl !text-foreground/50">{t('services.outdoor.subtitle')}</p>
          </div>
          ...
          <Marquee speed={0.4} className="-mx-4 px-4" gap={24}>
            {categories.map((cat, i) => (
              <motion.div
                ...
                className="shrink-0 w-[280px] md:w-auto p-6 md:p-8 rounded-[2rem] bg-background md:bg-foreground/[0.02] border border-border hover:border-primary/20 transition-all duration-500 group shadow-xl shadow-primary/5 hover:shadow-2xl hover:shadow-primary/5"
              >
                ...
                <h3 className="text-subtitle mb-4 group-hover:text-primary transition-colors duration-500 font-bold">{cat.title}</h3>
                <p className="text-sm md:text-label !text-foreground/60 leading-relaxed">
                  {cat.desc}
                </p>
              </motion.div>
            ))}
          ...
          <Marquee speed={0.4} className="-mx-4 px-4" gap={48}>
            {coordinationSteps.map((step, i) => (
              ...
              >
                ...
                <p className="text-sm md:text-label !text-foreground/80 px-4 font-medium">{step.body}</p>
              </motion.div>
            ))}
          </Marquee>
        </div>
            ...
            >
              ...
              <ul className="space-y-4 md:space-y-6">
                {t('services.boundaries.is.body').split('\n').map((item: string, i: number) => (
                  <li key={i} className="flex gap-4 items-center text-sm md:text-label !text-foreground/70">
            ...
            >
              ...
              <ul className="space-y-4 md:space-y-6">
                {t('services.boundaries.isNot.body').split('\n').map((item: string, i: number) => (
                  <li key={i} className="flex gap-4 items-center text-sm md:text-label !text-foreground/40">
      ...
      <section className="section-spacing bg-background overflow-hidden">
        <div className="container-sahli">
          <div className="grid lg:grid-cols-2 gap-12 lg:gap-16 items-center">
            <div>
              <h2 className="text-3xl md:text-display mb-8 md:mb-12 font-black">
                {t('home.areas.title')}
              </h2>
              <Marquee speed={0.4} className="-mx-4 px-4" gap={16}>
                {areas.map((area, i) => (
                  <motion.div
                    ...
                    viewport={{ once: true }}
                    transition={{ delay: i * 0.1 }}
                    className="shrink-0 w-[240px] md:w-auto flex items-center gap-4 md:gap-6 p-5 md:p-6 rounded-2xl bg-background md:bg-foreground/[0.02] border border-border hover:border-primary/30 transition-all group shadow-xl shadow-primary/5 hover:shadow-md"
                  >
                    ...
                    <span className="text-base md:text-subtitle font-bold group-hover:text-primary transition-colors">{area}</span>
                  </motion.div>
                ))}
              </Marquee>
            </div>
            <div className="relative aspect-square rounded-[2.5rem] md:rounded-[3rem] overflow-hidden border border-border bg-foreground/[0.02] flex items-center justify-center shadow-2xl shadow-primary/5">
              ...
              <div className="relative text-center p-8 md:p-12">
                ...
                <h2 className="text-2xl md:text-display mb-4 font-black">{t('services.areas.qatarNationwide')}</h2>
                <p className="text-sm md:text-label !text-foreground/60 font-medium">{t('services.areas.rapidResponse')}</p>
              </div>
            </div>
          </div>
        </div>
      ...
      <section className="py-16 md:py-24 bg-foreground/[0.02] border-t border-border overflow-hidden">
        <div className="container-sahli">
          <div className="mb-12 md:mb-16 text-center md:text-start">
            ...
            <p className="text-sm md:text-label !text-foreground/50">
