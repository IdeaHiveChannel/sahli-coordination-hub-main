              ...
              <ul className="space-y-2.5">
                {t('services.boundaries.isNot.body').split('\n').map((item: string, i: number) => (
                  <li key={i} className="flex gap-3 items-center text-xs text-slate-400 group">
                    <div className="w-1 h-1 rounded-full bg-slate-300 shrink-0 group-hover:scale-150 transition-transform duration-300" />
              ...
              <Marquee speed={0.4} className="-mx-4 px-4" gap={12}>
                {areas.map((area, i) => (
                  <ScrollReveal
                    ...
                    direction={dir === 'rtl' ? 'left' : 'right'}
                    delay={i * 0.1}
                    className="flex items-center gap-3 p-2.5 rounded-xl bg-white border border-slate-200 hover:border-primary/30 transition-all group shrink-0 w-[180px] md:w-auto shadow-xl shadow-primary/5"
                  >
                    ...
                    <span className="text-xs font-bold text-slate-600 group-hover:text-primary transition-colors duration-500">{area}</span>
                  </ScrollReveal>
                ))}
              </Marquee>
            </div>
            
            <div className="relative aspect-video lg:aspect-square rounded-xl overflow-hidden border border-slate-200 bg-slate-50 flex items-center justify-center shadow-2xl shadow-primary/5 group">
              <img 
              ...
              <div className="absolute inset-0 flex items-center justify-center">
                <div className="px-8 py-4 bg-white/90 backdrop-blur-xl rounded-xl border border-white/20 shadow-lg text-xs !text-primary font-bold">
        ...
        <div className="container-sahli">
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3 md:gap-4">
            {relatedServices.map((service, i) => (
              ...
              >
                <span className="text-xs font-bold text-slate-700 group-hover:text-primary transition-colors line-clamp-2 uppercase tracking-wider">{service.title}</span>
